...
impl QueryPlaneStore for SimEventStore {
    ...
    async fn query_field_index_page(
        ...
        // The retained projection map can enumerate candidates, but it does not
        // implement the SQL-shaped predicate/order contract of native paging.
        // Returning None selects the common evaluator, which applies typed OData
        // filtering and ordering before it truncates the page.
        Ok(None)
    }
    ...
    async fn projected_entity_counts_by_tenant(
        ...
        Ok(Some(self.query_projection_counts_by_tenant()))
    }
}
